...
export default function TeamLeadWorkspace(props) {
    ...
    // Same AJAX refetch as refresh(), just for a period switch — passes the
    // target period explicitly instead of reading `period` from state,
    // which wouldn't have updated yet within this same tick.
    async function setPeriod(key) {
        setRefreshing(true);
        try {
            const sep = data.dashboardDataUrl.includes('?') ? '&' : '?';
            const fresh = await apiFetch(`${data.dashboardDataUrl}${sep}period=${key}`);
            setData((prev) => ({ ...prev, ...fresh }));
            const params = new URLSearchParams(window.location.search);
    ...
    return (
        <div className="flex min-h-screen flex-col">
            ...
            {toast && (
                <div className="fixed bottom-6 left-1/2 z-[60] -translate-x-1/2 rounded-xl bg-gray-900 dark:bg-panel-selected px-4 py-2.5 text-[13px] font-semibold text-white shadow-lg">
                    {toast}
                </div>
            )}
        </div>
