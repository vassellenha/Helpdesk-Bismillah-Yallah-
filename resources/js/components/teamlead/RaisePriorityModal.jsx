...
export default function RaisePriorityModal({ ticket, remindUrlBase, onClose, onSaved }) {
    ...
    async function submit() {
        if (priority === ticket.priority) {
            setError(trans('teamlead.raise.same'));
            return;
        }
        setSaving(true);
        setError('');
        try {
            ...
            setError(e.message || trans('teamlead.raise.failed'));
            setSaving(false);
        }
    }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm" onMouseDown={onClose}>
            <div className="liquid-glass w-full max-w-sm overflow-hidden rounded-2xl shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
                <div className="flex items-start justify-between border-b border-gray-100 dark:border-edge px-5 py-4">
                    <div>
                        <h2 className="text-[15px] font-bold text-gray-900 dark:text-ink-1">{trans('teamlead.raise.title')}</h2>
                        <p className="mt-0.5 text-xs text-gray-400 dark:text-ink-3">{trans('teamlead.raise.current', { id: ticket.id, priority: ticket.priority })}</p>
                    </div>
                ...
                <div className="flex items-center justify-end gap-2 border-t border-gray-100 dark:border-edge px-5 py-3.5">
                    <button onClick={onClose} className="rounded-[10px] px-4 py-2.5 text-[13px] font-semibold text-gray-600 dark:text-ink-2 hover:bg-gray-100 dark:hover:bg-panel-hover">{trans('teamlead.common.cancel')}</button>
                    <button onClick={submit} disabled={saving} className="rounded-[10px] bg-blue-600 dark:bg-blue-500 px-4 py-2.5 text-[13px] font-semibold text-white hover:bg-blue-700 dark:hover:bg-blue-400 disabled:opacity-60">
                        {saving ? trans('teamlead.common.saving') : trans('teamlead.common.save')}
                    </button>
                </div>
            </div>
        </div>
