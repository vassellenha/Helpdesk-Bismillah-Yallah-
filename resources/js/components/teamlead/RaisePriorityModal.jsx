        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 p-4" onMouseDown={onClose}>
            <div className="w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
                <div className="flex items-start justify-between border-b border-gray-100 px-5 py-4">
                    <div>
                        <h2 className="text-[15px] font-bold text-gray-900">Ubah Prioritas</h2>
                        <p className="mt-0.5 text-xs text-gray-400">{ticket.id} · sekarang {ticket.priority}</p>
                    </div>
                        ...
                        {PRIORITIES.map((p) => (
                            <button
                                ...
                                className={`rounded-xl border px-3 py-2.5 text-[13px] font-semibold transition ${
                                    priority === p ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-700 hover:border-gray-300'
                                }`}
                            >
                                {p}
                            </button>
                ...
                <div className="flex items-center justify-end gap-2 border-t border-gray-100 px-5 py-3.5">
                    <button onClick={onClose} className="rounded-[10px] px-4 py-2.5 text-[13px] font-semibold text-gray-600 hover:bg-gray-100">Batal</button>
                    <button onClick={submit} disabled={saving} className="rounded-[10px] bg-blue-600 px-4 py-2.5 text-[13px] font-semibold text-white hover:bg-blue-700 disabled:opacity-60">
