    ...
    return (
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[320px_1fr]">
            <div className="rounded-2xl border border-gray-200 dark:border-edge-strong bg-white dark:bg-panel-2 p-5 shadow-sm">
                <h2 className="mb-4 text-[15px] font-bold text-gray-900 dark:text-ink-1">Report Generator</h2>

                <p className="mb-2 text-[12px] font-bold text-amber-700 dark:text-warn-text">Jenis Laporan</p>
                ...
                <p className="mb-2 text-[12px] font-bold text-amber-700 dark:text-warn-text">Periode</p>
                ...
                <p className="mb-2 text-[12px] font-bold text-amber-700 dark:text-warn-text">Unit Kerja</p>
                <div className="mb-5">
                    <SelectMenu
                        value={unit}
                        onChange={setUnit}
                        options={[{ value: 'Semua Unit', label: 'Semua Unit' }, ...reportUnits.map((u) => ({ value: u, label: u }))]}
                    />
                </div>
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-edge-strong bg-white dark:bg-panel-2 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 dark:border-edge p-5">
                    <div>
                        ...
                        <p className="mt-0.5 text-xs text-gray-400 dark:text-ink-3">Pratinjau · {from} → {to} · {unit}</p>
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            {report.rows.length === 0 && <tr><td colSpan={report.columns.length} className="px-5 py-12 text-center text-sm text-gray-400 dark:text-ink-3">Belum ada data untuk laporan ini.</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
