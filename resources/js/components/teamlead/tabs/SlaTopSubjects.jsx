    ...
    return (
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div className="flex flex-wrap items-baseline justify-between gap-2 p-5 pb-3">
                <div>
                    <h2 className="text-[15px] font-bold text-gray-900">SLA Compliance — Tiket Penyumbang Terbanyak</h2>
                    <p className="mt-0.5 text-xs text-gray-400">Subjek dengan volume tertinggi &amp; kontribusi breach terbesar</p>
                </div>
                <span className="text-[11px] font-bold text-gray-400">Bulan ini</span>
            </div>
            ...
            <div className="overflow-x-auto">
                <div className="min-w-[820px]">
                    <div className="grid grid-cols-[36px_1fr_120px_90px_80px_100px_36px] gap-3 border-y border-gray-100 bg-gray-50 px-6 py-2.5 text-[11px] font-bold uppercase tracking-wide text-gray-400">
                    ...
                    {pageRows.map((r, i) => {
                        ...
                        return (
                            <div key={r.subject} className="border-b border-gray-50 last:border-0">
                                <div
                                    onClick={() => setOpen(expanded ? null : r.subject)}
                                    className={`grid cursor-pointer grid-cols-[36px_1fr_120px_90px_80px_100px_36px] items-center gap-3 px-6 py-3.5 ${expanded ? 'bg-blue-50/40' : 'hover:bg-blue-50/30'}`}
                                >
                                    <span className="text-[12px] font-extrabold text-gray-400">{rank}</span>
                                    <div className="min-w-0">
                                        <p className="truncate text-[12.5px] font-semibold text-gray-900">{r.subject}</p>
                                        <div className="mt-1 flex items-center gap-1.5">
                                            <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold ${TYPE_STYLE[r.type] ?? TYPE_STYLE.Incident}`}>{r.type}</span>
                                            <span className="text-[10.5px] font-semibold text-gray-400">{r.subCat}</span>
                                        </div>
                                        <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-gray-100">
                                            <div className={`h-full rounded-full ${barColor(r.compliance)}`} style={{ width: `${Math.round((r.volume / maxVolume) * 100)}%` }} />
                                        </div>
                                    </div>
                                    <span className="truncate text-[11.5px] font-semibold text-gray-600">{r.app}</span>
                                    <span className="text-right text-[13px] font-extrabold text-gray-900">{r.volume}</span>
                                    <span className="text-right text-[13px] font-bold text-red-600">{r.breach}</span>
                                ...
                                {expanded && (
                                    <div className="flex flex-col gap-2 bg-gray-50/50 px-6 pb-4 pl-[70px] pt-1">
                                        <p className="text-[10.5px] font-bold uppercase tracking-wide text-gray-400">Rincian Masalah Tiket</p>
                                        {r.causes.map((c) => (
                                            <div key={c.name} className="flex items-center gap-3">
                                                <span className="flex-1 truncate text-[12px] text-gray-700">{c.name}</span>
                                                ...
                                                <span className="w-16 shrink-0 text-right text-[11.5px] font-bold text-gray-700">{c.volume} tiket</span>
                                                <span className="w-16 shrink-0 text-right text-[11px] font-bold text-red-600">{c.breach} breach</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                    ...
                    {pageRows.length === 0 && (
                        <div className="px-6 py-10 text-center text-sm text-gray-400">Tidak ada subjek yang cocok dengan pencarian.</div>
                    )}
                </div>
            </div>
            ...
            {filtered.length > PER_PAGE && (
                <div className="flex items-center justify-between gap-3 border-t border-gray-100 px-6 py-3.5">
                    ...
                    <div className="flex items-center gap-2.5">
                        <button onClick={() => goto(current - 1)} disabled={current <= 1} className="flex items-center gap-1 rounded-lg bg-gray-100 px-3 py-1.5 text-[12.5px] font-bold text-gray-700 disabled:text-gray-400">
                        ...
                        <span className="text-[12.5px] font-bold text-gray-700">Hal {current} / {pages}</span>
                        <button onClick={() => goto(current + 1)} disabled={current >= pages} className={`flex items-center gap-1 rounded-lg px-3.5 py-1.5 text-[12.5px] font-bold ${current >= pages ? 'bg-gray-100 text-gray-400' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
                            Berikutnya
