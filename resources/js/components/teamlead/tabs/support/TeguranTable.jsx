...
const TABS = [['all', 'teamlead.common.all'], ['email', 'teamlead.remind.email'], ['whatsapp', 'teamlead.remind.whatsapp'], ['none', 'teamlead.support.not_yet']];
        ...
        try {
            const res = await apiFetch(`${remindUrlBase}/${row.id}/remind`, {
                ...
                body: JSON.stringify({
                    ...
                    message: trans('teamlead.support.teguran_msg', { pic: row.pic, id: row.id, subject: row.subject }),
                }),
            });
            setItems((prev) => prev.map((t) => (t.id === row.id ? { ...t, channels: Array.from(new Set([...t.channels, channel])) } : t)));
            flash(res?.message ?? trans('teamlead.support.teguran_sent', { channel: CHANNEL[channel].label }));
            // Refetch the dashboard so the teguran shows up live in Riwayat.
            onSent?.();
        } catch (e) {
            flash(e.message || trans('teamlead.support.warn_failed'));
        } finally {
            setSending(null);
        }
    }
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-edge-strong bg-white dark:bg-panel-2 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[15px] font-bold text-gray-900 dark:text-ink-1">{trans('teamlead.support.teguran_table')}</h2>
                        <p className="mt-0.5 text-xs text-gray-400 dark:text-ink-3">{trans('teamlead.support.teguran_hint')}</p>
                    </div>
                </div>
                <div className="flex gap-1 rounded-xl bg-gray-100 dark:bg-panel-3 p-1">
                    {TABS.map(([k, l]) => (
                        <button key={k} onClick={() => setTab(k)} className={`rounded-lg px-3 py-1.5 text-[12px] font-bold transition ${tab === k ? 'bg-white dark:bg-panel-2 text-gray-900 dark:text-ink-1 shadow-sm' : 'text-gray-500 dark:text-ink-2'}`}>{trans(l)}</button>
                    ))}
                </div>
            </div>
            ...
            <div className="overflow-x-auto">
                <div className="min-w-[860px]">
                    <div className="grid grid-cols-[100px_1fr_130px_120px_190px_96px] gap-3 border-y border-gray-100 dark:border-edge bg-gray-50 dark:bg-panel-3 px-6 py-2.5 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-ink-3">
                        <span>{trans('teamlead.columns.ticket_id')}</span><span>{trans('teamlead.columns.subject')}</span><span>{trans('teamlead.columns.pic')}</span><span>{trans('teamlead.columns.created')}</span><span>{trans('teamlead.support.teguran')}</span><span className="text-right">{trans('teamlead.support.warn')}</span>
                    </div>
                    <div className="max-h-[460px] overflow-y-auto">
                        {filtered.map((t) => (
                            <div key={t.id} onClick={() => onOpenTicket?.(t.id)} className="group grid cursor-pointer grid-cols-[100px_1fr_130px_120px_190px_96px] items-center gap-3 border-b border-gray-50 dark:border-transparent dark:even:bg-white/[0.03] px-6 py-3.5 last:border-0 hover:bg-blue-50/30 dark:hover:bg-panel-hover">
                                ...
                                <div className="flex flex-wrap items-center gap-1.5">
                                    {t.channels.length === 0 && (
                                        <span className="flex items-center gap-1.5 text-[11.5px] font-semibold text-gray-400 dark:text-ink-3"><span className="h-1.5 w-1.5 rounded-full bg-gray-300" />{trans('teamlead.support.not_yet_warned')}</span>
                                    )}
                                    {t.channels.map((c) => (
                                ...
                                <div className="flex justify-end gap-1.5">
                                    <button onClick={(e) => { e.stopPropagation(); send(t, 'email'); }} disabled={sending} title={trans('teamlead.support.warn_email')} className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-50 dark:bg-accent-soft text-blue-600 dark:text-accent-text hover:bg-blue-100 dark:hover:bg-panel-hover disabled:opacity-50">
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); send(t, 'whatsapp'); }} disabled={sending} title={trans('teamlead.support.warn_whatsapp')} className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-50 dark:bg-ok-soft text-emerald-600 dark:text-ok-text hover:bg-emerald-100 dark:hover:bg-ok-soft disabled:opacity-50">
                        ...
                        {filtered.length === 0 && <div className="px-6 py-12 text-center text-sm text-gray-400 dark:text-ink-3">{trans('teamlead.support.teguran_empty')}</div>}
                    </div>
                </div>
            </div>

            {toast && (
                <div className="fixed bottom-6 left-1/2 z-[80] -translate-x-1/2 rounded-xl bg-gray-900 dark:bg-panel-selected px-4 py-2.5 text-[13px] font-semibold text-white shadow-lg">{toast}</div>
            )}
        </div>
    );
}
