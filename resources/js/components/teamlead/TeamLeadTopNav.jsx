...
const ICON_STYLES = {
    ...
    sla_teguran: { bg: 'bg-red-50', color: 'text-red-600' },
    ticket_escalated: { bg: 'bg-blue-50', color: 'text-blue-700' },
    ticket_approved: { bg: 'bg-emerald-50', color: 'text-emerald-600' },
    ticket_rejected: { bg: 'bg-red-50', color: 'text-red-600' },
};

export default function TeamLeadTopNav({ notifications = [], user = {}, dashboardUrl = '/', markAllReadUrl, onOpenTicket }) {
    ...
    function openNotification(n) {
        ...
        // Synthesized supervisor notifications carry a ticket_no → open the
        // slide-over in place; persisted ones fall back to their href.
            ...
            <div className="relative">
                ...
                >
                    ...
                    <span className="hidden text-left leading-tight sm:block">
                        <span className="block text-[13px] font-semibold text-gray-900">{user.name ?? 'Team Lead'}</span>
                    ...
                    <div className="absolute right-0 top-[54px] z-50 w-[250px] overflow-hidden rounded-2xl border border-gray-200 bg-white p-1.5 shadow-xl">
                        <div className="mb-1.5 flex items-center gap-2.5 border-b border-gray-50 px-3 py-2.5">
                            ...
                            <span className="min-w-0">
                                <span className="block truncate text-[13px] font-bold text-gray-900">{user.name ?? 'Team Lead'}</span>
                                <span className="block truncate text-[11px] text-gray-400">{user.title ?? ''}</span>
                            </span>
                        </div>
                        <a href={dashboardUrl} className="flex items-center gap-2.5 rounded-[9px] px-3 py-2.5 text-[13px] font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900">
                            Dashboard
                        </a>
