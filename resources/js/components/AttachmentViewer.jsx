            ...
            <div className={`flex flex-col gap-1.5 ${className}`}>
                {attachments.map((a) => (
                    <button
                        key={a.id}
                        type="button"
                        onClick={() => (isImage(a.name, a.url) ? setPreview(a) : window.open(a.url, '_blank', 'noopener'))}
                        className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2 text-left text-[13px] font-medium text-blue-600 hover:bg-gray-100 hover:underline"
                    >
                ...
                <div className="fixed inset-0 z-[70] flex items-center justify-center bg-gray-900/70 p-4" onClick={() => setPreview(null)}>
                    <div className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-4 py-3">
                            <button onClick={() => setPreview(null)} className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-[13px] font-bold text-gray-600 hover:bg-gray-100 hover:text-gray-900">
                            ...
                            <p className="min-w-0 flex-1 truncate text-center text-[13px] font-semibold text-gray-800">{preview.name}</p>
                            <a href={preview.url} target="_blank" rel="noreferrer" className="shrink-0 text-[12px] font-semibold text-blue-600 hover:underline">Buka penuh</a>
                        </div>
