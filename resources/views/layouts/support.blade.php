        ...
        <header class="sticky top-0 z-20 flex h-[62px] items-center gap-6 border-b border-gray-200 bg-white px-7">
            ...
            <nav class="flex items-center gap-1">
                <a
                    ...
                    class="flex items-center gap-2 rounded-[10px] px-3.5 py-2 text-[13px] font-semibold {{ request()->routeIs('dashboard.support') ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}"
                >
                ...
                <a
                    ...
                    class="flex items-center gap-2 rounded-[10px] px-3.5 py-2 text-[13px] font-semibold {{ request()->routeIs('support.tickets*') ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}"
                >
            ...
            <div
                ...
                data-props="{{ json_encode(['notifications' => $notifications ?? [], 'user' => $currentUser ?? [], 'inboxUrl' => route('dashboard.support'), 'ticketsUrl' => route('support.tickets'), 'markAllReadUrl' => route('support.notifications.read-all')]) }}"
            ></div>
        </header>
